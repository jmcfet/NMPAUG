/**
 * Scroller used to display the events that are related to main asset shown on the media card
 * @class $U.mediaCard.BTVMoreLikeThisScroller
 *
 * @constructor
...
$U.mediaCard.BTVMoreLikeThisScroller = ( function() {

	// The more like this scroller's name
	var SCROLLER_NAME = "BTVMoreLikeThisScroller";
	...
	function BTVMoreLikeThisScroller(container) {
		...
		if ($U.core.Device.isDesktop()) {
			...
			this._scroller = new $U.core.widgets.ShiftingAssetScroller(scroller, true, 1, SCROLLER_PADDING, {
				active : true,
			...
			}, SCROLLER_NAME, this._assetTileClickHandler, true);
		} else {
			this._scroller = new $U.core.widgets.ShiftingAssetScroller(scroller, true, 1, SCROLLER_PADDING, {
				active : true,
				indicatorContainerElement : scrollIndicatorContainer,
				indicatorElement : scrollIndicator
			}, null, null, SCROLLER_NAME, this._assetTileClickHandler, true);
		}
	...
	$N.apps.util.Util.extend(BTVMoreLikeThisScroller, superClass);
	var proto = BTVMoreLikeThisScroller.prototype;
	...
	proto.populateAssets = function(items) {
		...
		if (items !== this._scroller.getItems()) {
			this._scroller.populate(items, null, null, null, null, null, $U.core.Configuration.MORE_LIKE_THIS.BTV_EVENTS_TO_RENDER);
			this._items = items;
		}
	};
	...
	/**
	 * Updates the BTVMoreLikeThis scroller based on the provided asset.
	...
	proto.update = function(mediaCardAsset) {
		var headAsset = this._scroller.getHeadAsset();
		if (headAsset && headAsset.startTime && mediaCardAsset.startTime > headAsset.startTime) {
			while (this._scroller.getHeadAsset() && this._scroller.getHeadAsset().startTime !== mediaCardAsset.startTime) {
				this._scroller.shiftNextIntoView();
				this._scroller.setOnNowOnNext(this._scroller.getHeadAsset());
			}
		}
	};
	...
	return BTVMoreLikeThisScroller;
