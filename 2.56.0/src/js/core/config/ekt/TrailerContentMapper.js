...
$U.core.TrailerContentMapper = function() {
    ...
    proto.getDRMId = function() {
        if (contentObj._data && contentObj._data.technicals && contentObj._data.technicals[0] && contentObj._data.technicals[0].media) {
        	var media = contentObj._data.technicals[0].media;
        	if (media.AV_PlaylistName && media.AV_PlaylistName.drmId) {
        		return media.AV_PlaylistName.drmId;
        	}
        }
    };
    /**
    ...
    proto.getUri = function() {
        if (contentObj.media) {
	        return $U.core.Configuration.TRAILER_VIDEO_PATH + contentObj.media.AV_PlaylistName.fileName;
    	}
    	if (contentObj._data && contentObj._data.technicals && contentObj._data.technicals[0] && contentObj._data.technicals[0].media) {
    		return $U.core.Configuration.TRAILER_VIDEO_PATH + contentObj._data.technicals[0].media.AV_PlaylistName.fileName;
    	}
    };
    /**
